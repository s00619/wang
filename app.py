...
st.markdown("기본 정보와 본견적서 내용만 입력하면 **본견적서**, **가견적서(+5%)**, **타견적서(+10%)**가 백만 원 단위 절삭되어 자동으로 완성됩니다.")

# ---------------------------------------------------------
# Helper Functions
# ---------------------------------------------------------
def num2kor(num):
    """숫자를 한글 금액 표현으로 변환 (예: 30000000 -> 삼천만)"""
    if num == 0:
        return "영"
    units = ['', '만', '억', '조']
    sub_units = ['', '십', '백', '천']
...
def calculate_quote(items, rate, include_vat=True, cut_unit=1000000):
    """
    품목 리스트, 부가세 계산 및 총액까지 
    모든 단계에서 cut_unit(기본 백만 원, 1,000,000원) 미만 단수를 버림(절삭) 처리
    """
    if not items:
        return [], 0, 0, 0
...
cut_option = st.sidebar.selectbox(
    "금액 절삭(버림) 단위 선택",
    options=[1000000, 100000, 10000, 1],
    index=0, # 백만 원 단위 기본 선택
    format_func=lambda x: "백만 원 단위 절삭 (몇천 몇백만 원으로 정돈)" if x == 1000000 else ("십만 원 단위 절삭" if x == 100000 else ("만 원 단위 절삭" if x == 10000 else "절삭 없음"))
)
...
for idx, item in enumerate(st.session_state["items"]):
    ...
    item["amount"] = cols[3].number_input(f"금액(원) #{idx+1}", value=int(item["amount"]), step=1000000, key=f"amt_{idx}")
...
st.markdown("---")
st.subheader(f"📊 견적 금액 미리보기 [{vat_str}] - 백만 원 단위 절삭 적용")
...
if st.button("🚀 백만 원 단위 절삭 견적서 3종 엑셀 파일 생성하기", type="primary", use_container_width=True):
